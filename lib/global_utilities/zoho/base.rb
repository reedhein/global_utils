module GlobalUtilities
  module Zoho
    class Base
      ...
      include GlobalUtilities::Zoho::Concern::DB
      include GlobalUtilities::Zoho::Concern::Notes
        ...
        %w(Accounts Contacts Quotes Events Leads Potentials Tasks Users Notes Attachments).each do |klass|
          define_singleton_method klass.downcase.to_sym do
            ...
            api_return_objects.map do |aro|
              ...
              ['GlobalUtilities', 'Zoho', klass.singularize].join('::').constantize.new(api_zoho_object)
            end
          end
        end
      end
